...
- **Пульт акима** — карта настоящей Астаны (границы OpenStreetMap, офлайн SVG). Меры ставятся кликом; ещё до клика плашки районов показывают предсказанный результат (ghost-превью), а при выборе района на всех пяти районах видна ΔScore. У каждого района свой цвет, а в режимах легенды T1…C2 и Δ карта окрашивается по значениям показателей. Формула ТЗ `0.7 × D_avg + 0.3 × min D − N_crit` выписана на экране. Валидатор показывает 6 маркеров правил, у карточки меры одна причина блокировки, бюджет — 100 тиков.
...
|---|---|---|
| Тесты фронта и паритет двух движков | `cd web; npm test` | **143 passed**: эталоны 52.558 / 56.543 / 55.667 / 54.009 / 52.041 / 57.237, все 12 невалидных наборов дают ровно свой код, 71 кейс `golden.json` совпадает с Python до 1e-6 |
