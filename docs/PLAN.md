...
Цель — выиграть трек. Визуал — приоритет с первого часа, параллельно с движком и AI, а не «после».

## 0. Главная идея и что показываем жюри
...
- **Числа, которых нет у других.** Перцентиль среди всех 694 395 валидных наборов, оптимум 57.237, «20 003 плана хуже бездействия», худший 52.041, таймлайн 8 кварталов, выведенный из формулы лага (в Q8 — в точности формула ТЗ).
...
## 11. Опциональные возможности — «баллы за час» и стоп-правила

| # | Возможность | Часы | Критерий жюри | Стоп-правило |
|---|---|---|---|---|
...
| 11 | «Голоса районов»: 5 реплик по дельтам (быстрая модель) | 1 | запоминающийся момент | после 1–5 |
...
## 12. Сценарий демо на 3 минуты (таймкоды, числа, ключевые моменты)
...
| Время | На экране | Слова ведущего | Ключевой момент |
|---|---|---|---|
...
| **0:20–0:50** | Hover на M7 → пунктир `52.56 → …` до клика; клик M7 → what-if на 5 районах, Нура «лучший»; Esc. Пресет **«Наивный аким»**: 100/100, **54.01** (+1.45), «КРИТ 2» остаётся, Нура красная (только городская M12: 49.18 → 49.62) | «Вижу последствия до клика. Типичный аким даёт всё богатому району: сто у.е. потрачены, плюс полтора балла, Нура остаётся критической» | **момент 1: ghost + what-if** |
...
| **1:30–1:50** | «Прожить 8 кварталов» (`Space`): Q2 — M10, M12 включаются, синергия B1 +2; **Q4 — S1 Нуры 38.0 → 40.0**, штриховка гаснет in-place, `− N_crit 2` → `1`, «S1 40.0 ✓» (пауза 2 с); **Q6 — S2 35 → 40.25**, «КРИТ 0»; Q8 — Score 56.54. **Запасной вариант v1.0 (Timeline не смержен):** шаг пропускается, на Вердикте показываем DistrictRows Q0 → Q8, речь та же без «анимации» | «Школа с лагом 3 квартала даёт первый эффект в четвёртом — ровно 40.0. Кривая выведена из формулы лага: в Q8 число в точности совпадает с ТЗ» | **момент 2: красные зоны гаснут на карте** |
| **1:50–2:30** | «AI-анализ →»: Вердикт, шапка досчитывает 56.54, waterfall (меры + «синергия M10+M12» + «снятие критических +2.00»); **AgentTrace карточками** по SSE — серые серверные `validate ✓ · evaluate ✓ 56.54 · facts ✓`, затем агентские из реального ответа (например `best_neighbors ✓ {n}`); записка абзацами; hover на чип «+2.00» → столбик; бейдж **«N/N чисел подтверждены движком»**; рекомендация с проверенной дельтой → **«Применить»** → Пульт, Score пересчитывается | «AI не считает: агент получает таблицу фактов, вызывает движок как инструмент, когда хочет проверить альтернативу, и цитирует. Каждая рекомендация пересчитана сервером, каждое число сверено — вот бейдж. Применяем совет — Score сразу растёт» | **момент 3: записка + «Применить»** |
| **2:30–2:45** | Сравнение: Базовый 52.56 · Пример ТЗ 56.54 · Дешёвый 55.67 · Оптимум **57.24** · Ваш — маркеры на гистограмме 694 395. **Запасной вариант v1.0:** меню «Пресеты ▾» на Пульте с теми же числами и перцентиль-гистограмма в ScoreHero | «До оптимума из 694 395 сценариев — доли балла. Оптимум найден полным перебором, скрипт в репо» | перцентиль |
...
Если спросят «что если ключ перестанет работать» — переключатель «офлайн-режим» на полосе трассы. Если LLM не ответил — кэш срабатывает без паузы, речь не меняется.
...
| Нет интернета на площадке / ключ не работает на демо | `AI_CACHE=fallback`: `llm → cache → rules`, всегда 200; `data/cache/demo` в репо; репетиция без Wi-Fi; мобильная точка доступа |
...
| Ghost-превью работает рывками на слабом ноутбуке | debounce 60 мс; число без spring на ghost; `prefers-reduced-motion`; playback пропускается `Space` |
...
| Меньше людей/времени | точки сдачи после Этапов 2, 3, 4; вырезания до 30 ч и вариант «2 человека / 1 день» §7; запасной вариант демо v1.0 §12 |
...
- [ ] `docs/PLAN.md` = этот документ; `TASK_SPEC.md` без локальных путей и приватных ссылок; `ARCHITECTURE.md`, `DEMO.md` (с запасным вариантом v1.0), `PITCH.md` соответствуют коду.
...
- [ ] Все PR смержены, `main` = демонстрируемая версия, последний тег запушен, `git log` без мусора, коммиты по функциям от каждого участника.
